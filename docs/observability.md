...
- URL: `http://<host>:3000`
- Default credentials: `admin` / `admin` (override with `GRAFANA_ADMIN_USER` / `GRAFANA_ADMIN_PASSWORD` env vars)
...
For Docker Compose, set `GRAFANA_ADMIN_PASSWORD` in `.env` (see `.env.example`).
