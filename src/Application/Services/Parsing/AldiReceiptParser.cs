...
public partial class AldiReceiptParser : IReceiptParser
{
	public bool CanParse(string ocrText)
	{
		string header = ocrText.Length > 500 ? ocrText[..500] : ocrText;
		return AldiIdentifier().IsMatch(header);
	}
	...
	public ParsedReceipt Parse(string ocrText)
	{
		...
		FieldConfidence<string> storeName = FieldConfidence<string>.High("ALDI");
	...
	private static List<ParsedReceiptItem> ExtractItems(string[] lines)
	{
		...
		foreach (string line in lines)
		{
			...
			Match itemMatch = AldiItemPattern().Match(line);
	...
	[GeneratedRegex(@"\bALDI\b", RegexOptions.IgnoreCase)]
	private static partial Regex AldiIdentifier();

	[GeneratedRegex(@"\d{2}/\d{2}/\d{4}")]
	private static partial Regex DatePattern();

	[GeneratedRegex(@"^(?<desc>.+?)\s+(?<price>\d+\.\d{2})\s*$")]
	private static partial Regex AldiItemPattern();
