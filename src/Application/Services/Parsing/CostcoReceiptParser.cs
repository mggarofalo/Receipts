...
public partial class CostcoReceiptParser : IReceiptParser
{
	public bool CanParse(string ocrText)
	{
		string header = ocrText.Length > 500 ? ocrText[..500] : ocrText;
		return CostcoIdentifier().IsMatch(header);
	}
	...
	public ParsedReceipt Parse(string ocrText)
	{
		...
		FieldConfidence<string> storeName = FieldConfidence<string>.High("Costco");
	...
	private static List<ParsedReceiptItem> ExtractItems(string[] lines)
	{
		...
		foreach (string line in lines)
		{
			...
			Match itemMatch = CostcoItemPattern().Match(line);
			if (!itemMatch.Success)
			{
				continue;
			}

			string? itemCode = itemMatch.Groups["code"].Success ? itemMatch.Groups["code"].Value.Trim() : null;
			...
			FieldConfidence<string?> code = itemCode is not null
				? FieldConfidence<string?>.High(itemCode)
	...
	[GeneratedRegex(@"COSTCO(?:\s+WHOLESALE)?", RegexOptions.IgnoreCase)]
	private static partial Regex CostcoIdentifier();

	[GeneratedRegex(@"\d{2}/\d{2}/\d{4}")]
	private static partial Regex DatePattern();

	[GeneratedRegex(@"^(?:(?<code>\d{5,7})\s+)?(?<desc>.+?)\s+(?<price>\d+\.\d{2})\s*$")]
	private static partial Regex CostcoItemPattern();
