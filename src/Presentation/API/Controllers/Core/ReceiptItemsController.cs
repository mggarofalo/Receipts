...
public class ReceiptItemsController(IMediator mediator, ReceiptItemMapper mapper, ILogger<ReceiptItemsController> logger, IHubContext<ReceiptsHub, IReceiptsHubClient> hub) : ControllerBase
{
	...
	public async Task<ActionResult<ReceiptItemResponse>> CreateReceiptItem([FromBody] CreateReceiptItemRequest model, [FromRoute] Guid receiptId)
	{
		try
		{
			...
			ReceiptItemResponse response = mapper.ToResponse(receiptItems[0]);
			await hub.Clients.All.ReceiptItemCreated(response);
			return Ok(response);
		}
		catch (Exception ex)
		{
	...
	public async Task<ActionResult<List<ReceiptItemResponse>>> CreateReceiptItems([FromBody] List<CreateReceiptItemRequest> models, [FromRoute] Guid receiptId)
	{
		try
		{
			...
			List<ReceiptItem> receiptItems = await mediator.Send(command);
			List<ReceiptItemResponse> responses = receiptItems.Select(mapper.ToResponse).ToList();
			foreach (ReceiptItemResponse response in responses)
			{
				await hub.Clients.All.ReceiptItemCreated(response);
			}
			return Ok(responses);
	...
	public async Task<ActionResult<bool>> UpdateReceiptItem([FromBody] UpdateReceiptItemRequest model, [FromRoute] Guid receiptId)
	{
		try
		{
			...
			await hub.Clients.All.ReceiptItemUpdated(model.Id);
			return NoContent();
		}
		catch (Exception ex)
		{
	...
	public async Task<ActionResult<bool>> UpdateReceiptItems([FromBody] List<UpdateReceiptItemRequest> models, [FromRoute] Guid receiptId)
	{
		try
		{
			...
			foreach (UpdateReceiptItemRequest m in models)
			{
				await hub.Clients.All.ReceiptItemUpdated(m.Id);
			}
			return NoContent();
		}
		catch (Exception ex)
	...
	public async Task<ActionResult<bool>> DeleteReceiptItems([FromBody] List<Guid> ids)
	{
		try
		{
			...
			{
				await hub.Clients.All.ReceiptItemDeleted(id);
			}
			return NoContent();
		}
		catch (Exception ex)
	...
	public async Task<IActionResult> RestoreReceiptItem([FromRoute] Guid id)
	{
		try
		{
			...
			GetReceiptItemByIdQuery fetchQuery = new(id);
			ReceiptItem? restored = await mediator.Send(fetchQuery);
			if (restored != null)
			{
				await hub.Clients.All.ReceiptItemCreated(mapper.ToResponse(restored));
			}
			return NoContent();
		}
		catch (Exception ex)
