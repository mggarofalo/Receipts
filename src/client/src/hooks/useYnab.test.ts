...
describe("useYnab", () => {
  ...
  it("useStaleMappings returns stale counts on success", async () => {
    ...
    await waitFor(() => expect(result.current.isSuccess).toBe(true));
    expect(result.current.data?.staleAccountMappingCount).toBe(2);
    expect(result.current.data?.staleCategoryMappingCount).toBe(3);
    expect(result.current.data?.currentBudgetId).toBe("budget-1");
  });
